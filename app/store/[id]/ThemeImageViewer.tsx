...
// 미니 프리뷰 독립 컴포넌트 — 오른쪽 패널에서 사용
export function MiniPreviews({ themeId, images, previews, onHover }: {
    themeId?: number;
    images: string[];
    previews: string[];
    onHover?: (idx: number) => void;
}) {
    const { slots, useColor } = getPreviewSlots(themeId, images, previews);
    const [activeIdx, setActiveIdx] = useState(0);
        ...
        <div className="grid grid-cols-5 gap-2">
            {slots.slice(0, 5).map((slot, i) => (
                <button
                    ...
                    className="aspect-square rounded-[10px] overflow-hidden relative transition-all"
                    style={{
                ...
                >
                    {!useColor && typeof slot === "string" && (
                        <Image src={slot} alt={`preview-${i + 1}`} fill className="object-cover" />
                    )}
                </button>
            ))}
        </div>
