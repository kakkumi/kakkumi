...
export const ChatRoomScreen = React.memo(function ChatRoomScreen({ config, hideContent, bannerType }: { config: ScreenThemeConfig; hideContent?: boolean; bannerType?: 'notif' | 'directShare' }) {
  ...
  return (
    <div style={{
      ...
      {/* 알림 배너 (hideContent 모드) */}
      {hideContent && (
        <div style={{
          position: 'absolute', top: 52, left: 12, right: 12, zIndex: 30,
          backgroundColor: bannerType === 'directShare'
            ? (config.directShareBg ?? 'rgba(255,255,255,0.96)')
            : (config.notifBannerBg ?? 'rgba(255,255,255,0.96)'),
          borderRadius: 16,
          padding: '10px 14px',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1, minWidth: 0 }}>
            {bannerType === 'directShare' ? (
              <span style={{
                fontSize: 12, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                color: config.directShareMsgColor ?? '#3c3c3c',
              }}>
                <span style={{ color: config.directShareNameColor ?? '#191919' }}>서울</span>에게 사진을 전달하였습니다.
              </span>
            ) : (
              <>
                <span style={{
                  fontSize: 13, fontWeight: 600, lineHeight: 1,
                  color: config.notifBannerNameColor ?? '#191919',
                }}>서울</span>
                <span style={{
                  fontSize: 12, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                  color: config.notifBannerMsgColor ?? '#3c3c3c',
                }}>ㅋㅋㅋㅋㅋㅋㅋ</span>
              </>
            )}
          </div>
